...
export function connect(
  ...
  actions,
  mergeProps,
  { mapProps = identity, ...opts } = { mapProps: identity }
) {
  ...
  const mapActions = (dispatch, { store }) =>
    mapValues(actions, v => bindAction(store, dispatch, v))
  const selector = isPlainObject(query)
    ? composeSelector(query, mapProps)
    : query
  ...
  const connectHOC = standardConnect(
    selector,
    makeCallbacks,
    mergeProps,
    options
